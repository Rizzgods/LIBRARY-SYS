    ...
    function showContent(contentId) {
        ...
        // Set the selected state for the navigation link
        var navLinks = document.getElementsByClassName('nav-link');
        for (var j = 0; j < navLinks.length; j++) {
            navLinks[j].classList.remove('selected'); // Remove selected class from all links
        }

        // Add selected class to the clicked link
        var selectedLink = document.querySelector(`.nav-link[onclick="showContent('${contentId}')"]`);
        if (selectedLink) {
            selectedLink.classList.add('selected');
        }

        // Save selected content in localStorage
        localStorage.setItem('selectedContent', contentId);
    }

    window.onload = function() {
        var selectedContent = localStorage.getItem('selectedContent') || 'home';

        // Check if there's a URL hash to load the correct tab
        if (window.location.hash) {
            selectedContent = window.location.hash.substring(1); // Get hash without '#'
        }

        // Show the selected or default content and set the selected class
